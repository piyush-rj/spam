...
// Subscription key format (room:event)
type SubscriptionKey = string;

export class WebSocketClass {
  private wss: WebSocket.Server;
  private clients: Map<ClientId, WebSocket> = new Map();
  private subscriptions: Map<SubscriptionKey, Set<ClientId>> = new Map();
  ...
  private init(): void {
    this.wss.on('connection', (ws: WebSocket) => {
      ...
      // Send clientId to the client for reference
    ...
    switch (message.type) {
      ...
      case 'unsubscribe':
        if (message.room && message.event) {
          ...
          // Unsubscribe from all events in a room
          this.unsubscribeClientFromRoom(clientId, message.room);
        } else {
          // Unsubscribe from all (returning to dashboard)
