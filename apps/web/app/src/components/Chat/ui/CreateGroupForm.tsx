  ...
  async function handleSubmit() {
    try {
      ...
      if (!result.success) {
        ...
        let titleError = formatted.title?._errors?.[0];
        let passcodeError;
  
        if (validationResult.type === "PRIVATE") {
          passcodeError = (formatted as any).passcode?._errors?.[0];
        }
      ...
      // Prepare request payload properly
      const requestData: {
        title: string;
        type: string;
        passcode?: string;
      ...
      // Only add passcode if it's a private group
      if (isPrivate) {
          ...
          <div className="bg-[#141414] border-t border-l border-[#141414] p-8 rounded-lg shadow-lg w-96">
            ...
            <div className="flex justify-center mb-6">
              <div className="flex items-center">
                <div className={`w-6 h-6 rounded-full flex items-center justify-center ${step === 1 ? "bg-blue-600" : "bg-gray-600"}`}></div>
                <div className="w-12 h-1 bg-gray-600"></div>
                <div className={`w-6 h-6 rounded-full flex items-center justify-center ${step === 2 ? "bg-blue-600" : "bg-gray-600"}`}></div>
              </div>
            </div>
            {/* Error message */}
            {error && (
            ...
            {step === 2 && (
              <>
                ...
                <div className="flex justify-between">
                  <button
                    onClick={handleBack}
                    className="px-4 py-1.5 bg-blue-800 text-white rounded-md hover:bg-blue-950 transition-all duration-200"
                  >
                    Back
                  </button>
                  <button
